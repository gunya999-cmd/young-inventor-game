...
const STAGE_VERSION = 'workshop-stage-02-v5-2_75d';
const FIXED_DT = 1 / 120;
const XY_MIN = new THREE.Vector2(-5.35, 0.18);
const XY_MAX = new THREE.Vector2(4.35, 3.85);

type AssetKey = 'ramp' | 'lever' | 'pulley' | 'platform';
type DepthLayer = 'back' | 'main' | 'front';
type Placeable = {
  type: AssetKey;
...
type StageCanvas = HTMLCanvasElement & {
  ...
  __partWorldPosition?: (type: AssetKey) => { x: number; y: number; z: number; layer: DepthLayer } | null;
  __setPartLayer?: (type: AssetKey, layer: DepthLayer) => void;
};

const LAYER_Z: Record<DepthLayer, number> = { back: -0.52, main: 0, front: 0.52 };
const LAYER_LABEL: Record<DepthLayer, string> = { back: 'ЗАДНИЙ', main: 'MAIN', front: 'ПЕРЕДНИЙ' };
const LAYER_ORDER: DepthLayer[] = ['back', 'main', 'front'];
const ZONES: Zone[] = [
  { type: 'ramp', position: new THREE.Vector3(-3.55, 1.05, 0), radius: 1.18 },
  { type: 'lever', position: new THREE.Vector3(-0.75, 0.74, 0), radius: 0.95 },
  { type: 'platform', position: new THREE.Vector3(0.76, 0.50, 0), radius: 0.90 },
  { type: 'pulley', position: new THREE.Vector3(2.20, 2.55, 0), radius: 1.00 },
];
...
export async function installWorkshopStage02V5(): Promise<void> {
  await RAPIER.init();
  ...
  document.body.innerHTML = `
    <main class="aaa-workshop aaa-workshop--275d">
      <section class="aaa-loading"><span class="aaa-loading__gear">⚙</span><strong>Открываем лабораторию изобретателя…</strong><small>Full 3D visuals · 2.75D physics workspace</small></section>
      <header class="aaa-topbar">
        <div class="aaa-stage-badge"><small>STAGE</small><strong>02</strong><span>БАЛАНС И РЕАКЦИЯ</span></div>
        <div class="aaa-goal"><small>ЗАДАЧА</small><strong>Собери цепочку и запусти финальную машину</strong><span>X/Y — механизм · FRONT/MAIN/BACK — глубина</span></div>
        <div class="aaa-top-actions"><button data-action="camera">⌂ Вид</button><button data-action="reset">↻ Сброс</button><button class="aaa-play" data-action="play" disabled>▶ PLAY</button></div>
      </header>
      <section class="aaa-view">
        <canvas aria-label="Bright child friendly 2.75D physics workshop" data-stage-version="${STAGE_VERSION}" data-stage-state="loading"></canvas>
        <aside class="aaa-objectives"><strong>ЦЕЛИ</strong><ol>
          <li data-step="build"><i>1</i><span>Собери механизм в MAIN</span></li>
        ...
        <aside class="aaa-physics-card"><span>✦</span><div><strong>2.75D рабочая плоскость</strong><small>двигай X/Y · глубину меняй слоями</small></div></aside>
        <div class="aaa-depth-legend"><span data-depth="back">BACK</span><span class="is-active" data-depth="main">MAIN</span><span data-depth="front">FRONT</span></div>
        <div class="aaa-selection aaa-selection--275d" hidden>
          ...
          <div class="aaa-layer-picker" role="group" aria-label="Слой глубины">
            <button data-layer="back">BACK</button><button data-layer="main">MAIN</button><button data-layer="front">FRONT</button>
          </div>
          <button data-edit="delete" aria-label="Удалить">✕</button>
        </div>
        <section class="aaa-win" hidden><div>★</div><small>STAGE 02 COMPLETE</small><h2>Машина заработала!</h2><p>Ты собрал механизм в понятной плоскости, а физика осталась настоящей: масса, момент силы, столкновения и гравитация.</p><button data-action="again">Собрать ещё раз</button></section>
      ...
      <footer class="aaa-inventory">
        ...
        <div class="aaa-hint"><strong>УПРАВЛЕНИЕ</strong><span>1 палец: X/Y · 2 пальца: zoom</span></div>
      </footer>
    </main>`;
  ...
  style.textContent = `
    .aaa-workshop--275d .aaa-part,.aaa-workshop--275d .aaa-inventory{-webkit-user-select:none;user-select:none;-webkit-touch-callout:none}
    .aaa-workshop--275d .aaa-part{touch-action:none}
    .aaa-workshop--275d .aaa-view canvas{touch-action:none}
    .aaa-workshop--275d .aaa-part.is-dragging{transform:scale(.97)}
    .aaa-depth-legend{position:absolute;right:18px;top:76px;display:flex;gap:4px;padding:5px;border-radius:13px;background:rgba(255,255,255,.88);box-shadow:0 8px 24px rgba(43,97,132,.12);backdrop-filter:blur(10px)}
    .aaa-depth-legend span{padding:5px 8px;border-radius:8px;font-size:9px;font-weight:900;color:#7890a0}.aaa-depth-legend .is-active{background:#e4f4ff;color:#167eba}
    .aaa-selection--275d{max-width:min(720px,92vw);flex-wrap:wrap;justify-content:center}.aaa-layer-picker{display:flex;gap:3px;padding:3px;border-radius:11px;background:#eef7fc}.aaa-layer-picker button{width:auto!important;min-width:54px;padding:0 7px;font-size:9px}.aaa-layer-picker button.is-active{background:#1d88c8;color:#fff}
    @media(max-width:900px){.aaa-depth-legend{right:10px;top:10px}.aaa-selection--275d{bottom:10px}.aaa-layer-picker button{min-width:44px;padding:0 5px;font-size:8px}}
  `;
  document.head.appendChild(style);

  const renderer = new THREE.WebGLRenderer({ canvas, antialias: !qaMode, powerPreference: 'high-performance' });
  renderer.setPixelRatio(qaMode ? 0.7 : Math.min(devicePixelRatio, 1.7));
  ...
  scene.fog = new THREE.Fog(0xdff3ff, 18, 31);

  // Deliberately near-front camera: the player reads one clean XY mechanism plane,
  // while a small perspective angle and three depth layers preserve real 3D volume.
  const camera = new THREE.PerspectiveCamera(38, 1, 0.1, 70);
  const homePosition = new THREE.Vector3(0.85, 2.75, 11.4);
  const homeTarget = new THREE.Vector3(-0.35, 1.70, 0);
  ...
  windowFill.position.set(5.2, 3.4, -3.2); windowFill.lookAt(0, 1.3, 0); scene.add(windowFill);
  const warmFill = new THREE.PointLight(0xffc568, 12, 8, 2);
  warmFill.position.set(-2.2, 4.2, -1.4); scene.add(warmFill);

  scene.add(createWorkshopEnvironment());

  // A subtle vertical workboard explains the gameplay plane without looking like CAD.
  const workboard = new THREE.Mesh(
    new THREE.PlaneGeometry(11.2, 4.55),
    new THREE.MeshPhysicalMaterial({ color: 0xf5fbff, transparent: true, opacity: 0.11, roughness: 0.64, metalness: 0, side: THREE.DoubleSide, depthWrite: false })
  );
  workboard.position.set(-0.35, 1.95, -0.82);
  scene.add(workboard);
  ...
  const screenRay = (x: number, y: number): void => {
    const rect = canvas.getBoundingClientRect();
    pointer.set((x - rect.left) / rect.width * 2 - 1, -((y - rect.top) / rect.height * 2 - 1));
    raycaster.setFromCamera(pointer, camera);
  };
  const screenToLayerPlane = (x: number, y: number, layer: DepthLayer): THREE.Vector3 | null => {
    const rect = canvas.getBoundingClientRect();
    if (rect.width <= 0 || rect.height <= 0) return null;
    screenRay(x, y);
    const plane = new THREE.Plane(new THREE.Vector3(0, 0, 1), -LAYER_Z[layer]);
    const point = new THREE.Vector3();
    if (!raycaster.ray.intersectPlane(plane, point)) return null;
    point.x = THREE.MathUtils.clamp(point.x, XY_MIN.x, XY_MAX.x);
    point.y = THREE.MathUtils.clamp(point.y, XY_MIN.y, XY_MAX.y);
    point.z = LAYER_Z[layer];
    return point;
  };
  const xyDistance = (a: THREE.Vector3, b: THREE.Vector3): number => Math.hypot(a.x - b.x, a.y - b.y);
  const applyLayer = (part: Placeable, layer: DepthLayer): void => {
    part.layer = layer;
    part.object.position.z = LAYER_Z[layer];
    part.snapped = false;
    canvas.dataset.selectedLayer = layer;
  };
  const snap = (part: Placeable): void => {
    const zone = zoneFor(part.type);
    part.snapped = part.layer === 'main' && xyDistance(part.object.position, zone.position) < zone.radius;
    if (part.snapped) {
      part.object.position.set(zone.position.x, zone.position.y, 0);
      part.layer = 'main';
      part.rotationZ = part.type === 'ramp' ? -0.19 : 0;
      part.object.rotation.set(0, 0, part.rotationZ);
    }
  };
  const buildReady = (): boolean => ZONES.every((zone) => placeables.some((part) => part.type === zone.type && part.snapped && part.layer === 'main'));
  const updateLayerUi = (): void => {
    root.querySelectorAll<HTMLElement>('[data-layer]').forEach((button) => button.classList.toggle('is-active', !!selected && button.dataset.layer === selected.layer));
    root.querySelectorAll<HTMLElement>('[data-depth]').forEach((item) => item.classList.toggle('is-active', item.dataset.depth === (selected?.layer ?? 'main')));
    canvas.dataset.selectedLayer = selected?.layer ?? 'none';
  };
  const select = (part: Placeable | null): void => {
    ...
    if (label && part) label.textContent = `${LABELS[part.type]} · ${LAYER_LABEL[part.layer]}`;
    updateLayerUi();
  };
  const refresh = (): void => {
  ...
  const beginInventoryDrag = (event: PointerEvent, type: AssetKey, button: HTMLButtonElement): void => {
    ...
    try { button.setPointerCapture(event.pointerId); } catch { /* Safari synthetic events */ }
    button.classList.add('is-dragging');
    const object = makeGhost(type);
    object.position.z = 0;
    drag = { pointerId: event.pointerId, pointerType: pointerKind(event), type, source: 'inventory', object, part: null, offset: new THREE.Vector3(), insideCanvas: false };
  ...
  const beginSceneDrag = (event: PointerEvent, part: Placeable, point: THREE.Vector3): void => {
    ...
    drag = {
      ...
      pointerType: pointerKind(event),
      type: part.type,
      source: 'scene',
      object: part.object,
      part,
      offset: part.object.position.clone().sub(point),
      insideCanvas: true,
    };
    controls.enabled = false;
    canvas.dataset.dragging = 'true';
  ...
  const moveActiveDrag = (event: PointerEvent): void => {
    ...
    const layer = drag.part?.layer ?? 'main';
    const point = screenToLayerPlane(event.clientX, event.clientY, layer);
    if (!point) return;
    ...
    if (layer === 'main') {
      ...
      if (xyDistance(drag.object.position, zone.position) < zone.radius * 0.68) {
        drag.object.position.x = THREE.MathUtils.lerp(drag.object.position.x, zone.position.x, 0.65);
        drag.object.position.y = THREE.MathUtils.lerp(drag.object.position.y, zone.position.y, 0.65);
      }
    }
  };
  const endActiveDrag = (event: PointerEvent): void => {
    ...
    if (current.source === 'inventory') {
      ...
      if (current.insideCanvas) {
        ...
        object.visible = true;
        scene.add(object);
        const part: Placeable = { type: current.type, object, snapped: false, rotationZ: 0, layer: 'main' };
        snap(part); placeables.push(part); select(part);
      }
    } else if (current.part) {
      snap(current.part); select(current.part);
    }
    refresh();
  };

  root.querySelectorAll<HTMLButtonElement>('.aaa-part').forEach((button) => button.addEventListener('pointerdown', (event) => beginInventoryDrag(event, button.dataset.part as AssetKey, button)));
  ...
  canvas.addEventListener('pointerdown', (event) => {
    ...
    if (part) {
      const point = screenToLayerPlane(event.clientX, event.clientY, part.layer);
      if (point) beginSceneDrag(event, part, point);
    } else select(null);
  }, { capture: true });
  ...
  const canonical = (): void => {
    ...
    for (const zone of ZONES) {
      ...
      const rotationZ = zone.type === 'ramp' ? -0.19 : 0;
      object.position.copy(zone.position); object.rotation.set(0, 0, rotationZ); scene.add(object);
      placeables.push({ type: zone.type, object, snapped: true, rotationZ, layer: 'main' });
    }
    select(null); refresh();
  };
  ...
  canvas.__partWorldPosition = (type: AssetKey) => {
    const part = placeables.find((item) => item.type === type); if (!part) return null;
    return { x: part.object.position.x, y: part.object.position.y, z: part.object.position.z, layer: part.layer };
  };
  canvas.__setPartLayer = (type: AssetKey, layer: DepthLayer): void => {
    const part = placeables.find((item) => item.type === type); if (!part || simulation) return;
    applyLayer(part, layer); select(part); refresh();
  };
  ...
  root.addEventListener('click', (event) => {
    ...
    const layer = target.closest<HTMLElement>('[data-layer]')?.dataset.layer as DepthLayer | undefined;
    ...
    if (layer && selected && !simulation && LAYER_ORDER.includes(layer)) { applyLayer(selected, layer); select(selected); refresh(); }
    if (edit && selected && !simulation) {
      if (edit === 'left') selected.rotationZ += Math.PI / 12;
      if (edit === 'right') selected.rotationZ -= Math.PI / 12;
      if (edit === 'delete') { scene.remove(selected.object); placeables.splice(placeables.indexOf(selected), 1); select(null); refresh(); return; }
      selected.object.rotation.set(0, 0, selected.rotationZ); selected.snapped = false; refresh();
    }
  });
  ...
  canvas.dataset.inputSystem = 'unified-pointer-events-2_75d-v2';
  canvas.dataset.touchReady = 'true';
  canvas.dataset.gameplaySpace = 'constrained-2.75d';
  canvas.dataset.activeAxes = 'x-y';
  canvas.dataset.depthLayers = 'back-main-front';
  canvas.dataset.cameraMode = 'limited-perspective';
  canvas.dataset.rotationAxis = 'z';
  canvas.dataset.dragging = 'false';
  canvas.dataset.placeableCount = '0';
  canvas.dataset.selectedLayer = 'none';
  ...
  const loop = (now: number): void => {
    ...
    if (simulation && canvas.dataset.stageState === 'running') {
      accumulator += dt;
      while (accumulator >= FIXED_DT && !simulation.state.goalPowered) { simulation.step(); accumulator -= FIXED_DT; }
      sync();
    }

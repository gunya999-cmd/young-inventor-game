...
const TARGET_DRIVE_SPEED = 10.5;
const MAX_DRIVE_TORQUE = 5.6;
const BELT_RESPONSE = 6.4;
const MAX_BELT_FORCE = 8.5;
const GENERATOR_DRAG = 0.055;
const LOAD_DRAG = 0.18;
const POWER_RPM_THRESHOLD = 3.0;
const FULL_OUTPUT_OMEGA = 8.0;

function clamp(value: number, min: number, max: number): number {
  return Math.max(min, Math.min(max, value));
}

function makeCable(points: THREE.Vector3[], radius: number, material: THREE.Material): THREE.Mesh {
  const curve = new THREE.CatmullRomCurve3(points);
  return new THREE.Mesh(new THREE.TubeGeometry(curve, 72, radius, 10, false), material);
}

function makePulley(radius: number, material: THREE.Material, grooveMaterial: THREE.Material): THREE.Group {
  const group = new THREE.Group();
  const wheel = new THREE.Mesh(new THREE.CylinderGeometry(radius, radius, 0.18, 64), material);
  wheel.rotation.x = Math.PI / 2;
  group.add(wheel);

  for (const z of [-0.095, 0.095]) {
    const rim = new THREE.Mesh(new THREE.TorusGeometry(radius * 0.96, 0.035, 12, 64), grooveMaterial);
    rim.position.z = z;
    group.add(rim);
  }

  const hub = new THREE.Mesh(new THREE.CylinderGeometry(radius * 0.22, radius * 0.22, 0.26, 40), grooveMaterial);
  hub.rotation.x = Math.PI / 2;
  group.add(hub);

  for (let index = 0; index < 6; index += 1) {
    const angle = (index / 6) * Math.PI * 2;
    const spoke = new THREE.Mesh(new THREE.BoxGeometry(0.055, radius * 1.35, 0.055), grooveMaterial);
...
function makeLamp(): { group: THREE.Group; bulb: THREE.Mesh; glow: THREE.PointLight } {
  const group = new THREE.Group();
  group.position.set(2.55, 0.20, 0.05);
  const dark = new THREE.MeshPhysicalMaterial({ color: 0x31383d, metalness: 0.78, roughness: 0.30 });
  const base = new THREE.Mesh(new THREE.CylinderGeometry(0.30, 0.36, 0.42, 40), dark);
...
function makeBelt(material: THREE.Material): THREE.Group {
  const group = new THREE.Group();
  const left = DRIVER_X;
  const right = GENERATOR_X;
  const y = PULLEY_Y;
  const r = PULLEY_RADIUS;
  const z = 0.87;

  const top = new THREE.Mesh(new THREE.BoxGeometry(right - left, 0.072, 0.105), material);
  top.position.set((left + right) / 2, y + r, z);
  group.add(top);
  const bottom = top.clone();
  bottom.position.y = y - r;
  group.add(bottom);

  for (const x of [left, right]) {
    const loop = new THREE.Mesh(new THREE.TorusGeometry(r, 0.052, 12, 72), material);
...
export function installGeneratorLab(): void {
  ...
  root.innerHTML = `
    <header class="bowling-ball-lab__header">
      ...
      <div class="bowling-ball-lab__meta"><span>PBR</span><span>Planck</span><span>fan-belt input</span><span>mechanical → electrical</span><span>v1</span></div>
    </header>
    <div class="bowling-ball-lab__stage">
      <canvas aria-label="Generator 3D preview" data-asset-version="generator-v1"></canvas>
  ...
  for (let index = 0; index < 10; index += 1) {
    ...
    vent.position.set(GENERATOR_X + Math.cos(angle) * 0.64, PULLEY_Y + Math.sin(angle) * 0.64, 0.42);
    vent.rotation.z = angle;
    assembly.add(vent);
    const coil = new THREE.Mesh(new THREE.BoxGeometry(0.038, 0.16, 0.03), copper);
    coil.position.set(GENERATOR_X + Math.cos(angle) * 0.60, PULLEY_Y + Math.sin(angle) * 0.60, 0.385);
  ...
  const generatorPulley = makePulley(PULLEY_RADIUS, darkMetal, machined);
  ...
  const driverPulley = makePulley(PULLEY_RADIUS, new THREE.MeshPhysicalMaterial({ color: 0x5a6267, metalness: 0.88, roughness: 0.24 }), machined);
  driverPulley.position.set(DRIVER_X, PULLEY_Y, 0.88);
  assembly.add(driverPulley);

  const belt = makeBelt(rubber);
  assembly.add(belt);
  ...
  const driverBody = world.createBody({ type: 'dynamic', position: Vec2(DRIVER_X, PULLEY_Y), angularDamping: 0.045, gravityScale: 0, allowSleep: false });
  driverBody.createFixture({ shape: Circle(PULLEY_RADIUS), density: 3.8, friction: 0.65, restitution: 0.01 });
  ...
  rotorBody.createFixture({ shape: Circle(PULLEY_RADIUS), density: 3.1, friction: 0.65, restitution: 0.01 });
  ...
  const renderOutput = (): void => {
    ...
    outputLevel = clamp((rotorOmega - POWER_RPM_THRESHOLD) / (FULL_OUTPUT_OMEGA - POWER_RPM_THRESHOLD), 0, 1);
    generatedPower = loadConnected && outputLevel > 0.04;

    const bulbMaterial = lamp.bulb.material as THREE.MeshPhysicalMaterial;
    bulbMaterial.emissive.setHex(generatedPower ? 0xffad42 : 0x000000);
    bulbMaterial.emissiveIntensity = generatedPower ? 0.45 + outputLevel * 3.5 : 0;
    lamp.glow.intensity = generatedPower ? 0.5 + outputLevel * 3.4 : 0;

    if (!loadConnected) {
      status.textContent = rotorOmega > POWER_RPM_THRESHOLD
        ? 'Генератор вращается · электрическая нагрузка отключена'
        : 'Нагрузка отключена · генератор не вырабатывает полезную мощность';
    } else if (generatedPower) {
      status.textContent = `Генерация ${(outputLevel * 100).toFixed(0)}% · лампа питается от вращения ремня`;
    } else if (driveEnabled) {
      status.textContent = 'Ременной привод раскручивает генератор…';
    } else {
      status.textContent = 'Привод остановлен · генератор не вырабатывает ток';
    }
  };

  const applyBeltAndGeneratorLoad = (): void => {
    const driverOmega = driverBody.getAngularVelocity();
    const rotorOmega = rotorBody.getAngularVelocity();
    beltSlip = driverOmega * PULLEY_RADIUS - rotorOmega * PULLEY_RADIUS;
    const beltForce = clamp(beltSlip * BELT_RESPONSE, -MAX_BELT_FORCE, MAX_BELT_FORCE);
    driverBody.applyTorque(-beltForce * PULLEY_RADIUS, true);
    rotorBody.applyTorque(beltForce * PULLEY_RADIUS, true);

    if (loadConnected) {
      const omega = rotorBody.getAngularVelocity();
      const electricalTorque = clamp(-omega * LOAD_DRAG, -1.85, 1.85);
      rotorBody.applyTorque(electricalTorque, true);
    }
  };
  ...
  resetButton.addEventListener('click', () => {
    ...
    beltSlip = 0;
    outputLevel = 0;
    generatedPower = false;
    renderOutput();
  });

  const camera = new THREE.PerspectiveCamera(30, 1, 0.1, 100);
  camera.position.set(0.35, 0.38, 9.4);
  camera.lookAt(0.05, -0.04, 0.20);
  ...
  const animate = (now: number): void => {
    const wallDt = Math.min(0.050, Math.max(0, (now - previous) / 1000));
    previous = now;
    accumulator = Math.min(0.25, accumulator + wallDt);

    while (accumulator >= FIXED_DT) {
      applyBeltAndGeneratorLoad();
    ...
    beltPhase += driverBody.getAngularVelocity() * wallDt * 0.08;
    belt.rotation.z = Math.sin(beltPhase) * 0.0007;
    renderOutput();
    ...
    canvas.dataset.physics = 'planck-finite-torque-driver+friction-belt+generator-load-v1';

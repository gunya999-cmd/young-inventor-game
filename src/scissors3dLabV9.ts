...
const ROPE_RENDER_Z = 0.0;
const ROPE_GRAVITY = -3.8;
...
function makeRopeNodes(): RopeNode[] {
...
function segmentYAtX(a: THREE.Vector2, b: THREE.Vector2, x: number): number | null {
  const dx = b.x - a.x;
...
export function installScissors3DLabV9(): void {
  ...
  topAnchor.position.set(ROPE_X, ROPE_TOP + 0.05, ROPE_RENDER_Z);
  assembly.add(topAnchor);
  const bottomAnchor = makeAnchorVisual();
  bottomAnchor.rotation.z = Math.PI / 2;
  bottomAnchor.position.set(ROPE_X, ROPE_BOTTOM - 0.05, ROPE_RENDER_Z);
  ...
  const setTube = (mesh: THREE.Mesh, points: THREE.Vector3[]): void => {
    ...
    mesh.geometry = new THREE.TubeGeometry(curve, Math.max(34, points.length * 5), ROPE_RADIUS, 10, false);
    old.dispose();
    mesh.visible = true;
  };
  ...
  const ropePoints3 = (from: number, toInclusive: number): THREE.Vector3[] => {
    ...
    for (let i = from; i <= toInclusive; i += 1) {
      points.push(new THREE.Vector3(ropeNodes[i].p.x, ropeNodes[i].p.y, ROPE_RENDER_Z));
    }
    return points;
  };
  ...
  const cuttingEdges = () => ({
    upperA: bodyPoint(upperBody, -2.96, -0.015),
    upperB: bodyPoint(upperBody, -0.48, -0.070),
    lowerA: bodyPoint(lowerBody, -2.96, 0.015),
    lowerB: bodyPoint(lowerBody, -0.48, 0.070),
  });

  const cutCorridorAtRope = (): { inside: boolean; centerY: number; gap: number } => {
    const edges = cuttingEdges();
    const upperY = segmentYAtX(edges.upperA, edges.upperB, ROPE_X);
    const lowerY = segmentYAtX(edges.lowerA, edges.lowerB, ROPE_X);
    if (upperY === null || lowerY === null) return { inside: false, centerY: 0, gap: Number.POSITIVE_INFINITY };
    const centerY = (upperY + lowerY) * 0.5;
    const gap = Math.abs(upperY - lowerY);
    const ropeCoversCenter = ropeNodes.some((node, i) => {
      if (i >= ropeNodes.length - 1 || !activeLinks[i]) return false;
      const a = node.p;
      const b = ropeNodes[i + 1].p;
      return Math.min(a.y, b.y) - ROPE_RADIUS <= centerY && Math.max(a.y, b.y) + ROPE_RADIUS >= centerY;
    });
    return { inside: ropeCoversCenter, centerY, gap };
  };

  const cutNearestLink = (y: number): void => {
    let best = -1;
    let bestDistance = Number.POSITIVE_INFINITY;
    for (let i = 0; i < ropeNodes.length - 1; i += 1) {
      if (!activeLinks[i]) continue;
      const midY = (ropeNodes[i].p.y + ropeNodes[i + 1].p.y) * 0.5;
  ...
  const defaultCamera = new THREE.Vector3(-0.15, 0.55, 9.7);

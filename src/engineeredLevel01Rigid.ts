...
export function installEngineeredLevel01Rigid():void{
  ...
  function build(){
    ...
    // Seesaw body: horizontal at rest. Ball lands LEFT of pivot.
    ...
    // Ball 2 sits on upper end of ramp2, blocked by a vertical stopper.
    ...
    // Physical linkage: when right end of seesaw rises it pushes a vertical follower,
    // represented by a light rigid body constrained to vertical motion, contacting stopper from below.
    ...
    // Horizontal rigid bar linking follower to stopper: distance maintained with a long thin body
    // that is allowed to rotate slightly at contacts. This makes the visible lift mechanically causal.
  ...
  function bodyPoly(b:Body,halfW:number,halfH:number,fill:string){const p=b.getPosition(),a=b.getAngle();ctx.save();ctx.translate(sx(p.x),sy(p.y));ctx.rotate(-a);ctx.fillStyle=fill;ctx.strokeStyle='#3d4446';ctx.lineWidth=2;ctx.fillRect(-sx(halfW),-sy(SCALE_Y-halfH)+sy(SCALE_Y),sx(halfW*2),halfH/SCALE_Y*canvas.clientHeight*2);ctx.strokeRect(-sx(halfW),-halfH/SCALE_Y*canvas.clientHeight,sx(halfW*2),halfH/SCALE_Y*canvas.clientHeight*2);ctx.restore();}
  ...
  function draw(){const cw=canvas.clientWidth,ch=canvas.clientHeight;if(!cw||!world)return;ctx.clearRect(0,0,cw,ch);ctx.fillStyle='#f2f3f1';ctx.fillRect(0,0,cw,ch);ctx.strokeStyle='#d9dcda';ctx.lineWidth=1;for(let x=.25;x<4;x+=.25){ctx.beginPath();ctx.moveTo(sx(x),0);ctx.lineTo(sx(x),ch);ctx.stroke();}for(let y=.25;y<2.25;y+=.25){ctx.beginPath();ctx.moveTo(0,sy(y));ctx.lineTo(cw,sy(y));ctx.stroke();}
    ...
    const sp=pos(seesaw),sa=seesaw.getAngle();ctx.save();ctx.translate(sx(sp.x),sy(sp.y));ctx.rotate(-sa);ctx.fillStyle='#b8bdbe';ctx.strokeStyle='#3d4446';ctx.lineWidth=2;ctx.fillRect(-sx(.43),-7,sx(.86),14);ctx.strokeRect(-sx(.43),-7,sx(.86),14);ctx.restore();drawCircle({getPosition:()=>pivot} as Body,.028,'#2f3638');

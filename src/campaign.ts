...
export function loadCampaignProgress(storage:Pick<Storage,'getItem'>=localStorage):CampaignProgress{
 try{const raw=storage.getItem(CAMPAIGN_PROGRESS_KEY);if(!raw)return{levels:{}};const parsed=JSON.parse(raw) as Partial<CampaignProgress>;return{levels:parsed.levels&&typeof parsed.levels==='object'?parsed.levels as Record<string,LevelProgress>:{}};}catch{return{levels:{}};}
}

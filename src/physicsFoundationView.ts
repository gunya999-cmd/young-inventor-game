...
export function installPhysicsFoundation():void{
  ...
  function updateCopy(){
    ...
    else{title.textContent='LEVEL 01 · РЫЧАГ';copy.innerHTML='<p><b>Цель:</b> поднять правое плечо качелей до контрольной зоны.</p><ol><li>Шар катится с левой рампы.</li><li>Он давит на левое плечо.</li><li>Качели вращаются вокруг настоящего revolute joint.</li><li>Правое плечо входит в sensor-зону цели.</li></ol><p>Sensor только фиксирует касание и не воздействует на механизм.</p>';footer.textContent='ЦЕПЬ: гравитация → импульс шара → момент на оси → подъём правого плеча → sensor goal';}
  }
  ...
  document.querySelector('#pf-run')!.addEventListener('click',()=>{scene=buildFoundationLevel(level);running=true;won=false;status.textContent='Симуляция идёт: движок считает реальные контакты.';});
  document.querySelector('#pf-reset')!.addEventListener('click',()=>setLevel(level));

  function step(dt:number){if(!running)return;let left=Math.min(dt,.05);const fixed=1/120;while(left>0){scene.world.step(fixed);left-=fixed;}if(foundationSuccess(level,scene)){won=true;running=false;status.textContent='✓ PASS — физическое тело вошло в цель.';}const p=scene.ball.getPosition();if(!won&&p.y<-.3){running=false;status.textContent='✗ FAIL — тело покинуло рабочую область.';}}
  ...
  function draw(){if(!canvas.clientWidth)return;drawGrid();const bp=scene.ball.getPosition();if(level===0){line([.35,1.72],[2.55,.46],13);line([2.48,.18],[3.42,.18],7);line([2.50,.18],[2.50,.60],7);line([3.40,.18],[3.40,.60],7);circle(bp.x,bp.y,.07);ctx.fillStyle='#303638';ctx.font='700 14px system-ui';ctx.fillText('START',sx(.28),sy(2.00));ctx.fillText('GOAL',sx(2.72),sy(.70));}
    else{line([.32,1.70],[1.08,1.16],12);const seesaw=scene.seesaw!,sp=seesaw.getPosition(),a=seesaw.getAngle();ctx.save();ctx.translate(sx(sp.x),sy(sp.y));ctx.rotate(-a);ctx.fillStyle='#b7bcbd';ctx.strokeStyle='#3d4446';ctx.lineWidth=2;ctx.fillRect(-px(.62),-py(.04),px(1.24),py(.08));ctx.strokeRect(-px(.62),-py(.04),px(1.24),py(.08));ctx.restore();circle(1.48,.88,.028,'#303638');circle(bp.x,bp.y,.07);const tp=scene.target!.getPosition();circle(tp.x,tp.y,.065,scene.targetHit?'#d8dcda':'#eceeeb');ctx.fillStyle='#303638';ctx.font='700 13px system-ui';ctx.fillText('ШАР',sx(.34),sy(1.95));ctx.fillText('ЛЕВОЕ ПЛЕЧО ↓',sx(.86),sy(.70));ctx.fillText('ПРАВОЕ ПЛЕЧО ↑',sx(1.58),sy(1.24));ctx.fillText('SENSOR ЦЕЛЬ',sx(tp.x-.15),sy(tp.y+.16));}

...
const assets = [
  ['boxing-glove', 'boxing-glove-v9', 'CC-BY', 'sketchfab-incg5764-boxing-glove-cc-by'],
...
for (const [asset, version, license, sourceKey] of assets) {
  test(`${asset} premium 3D lab fits on a phone viewport`, async ({ page }) => {
    ...
    if (asset === 'boxing-glove') {
      ...
      await page.screenshot({ path: 'test-results/boxing-glove-v9-impulse.png', fullPage: true });
      ...
      await expect.poll(async () => Number(await canvas.getAttribute('data-speed')))
        .toBeLessThan(0.06);
      await page.screenshot({ path: 'test-results/boxing-glove-v9-settled.png', fullPage: true });
    }
  });
}

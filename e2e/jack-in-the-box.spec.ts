...
test('classic Part 14 Jack-in-the-Box loads the real CC0 source mesh and keeps Planck release physics', async ({ page }) => {
  ...
  await expect(canvas).toHaveAttribute('data-asset-version', 'jack-in-the-box-v4-real-cc0');
  await expect(canvas).toHaveAttribute('data-source-license', 'CC0-1.0');
  await expect(canvas).toHaveAttribute('data-source-key', 'meshy-cc0-rusted-jack-in-the-box');
  await expect(canvas).toHaveAttribute('data-render-source', 'real-cc0-glb');
  ...
  await expect(canvas).toHaveAttribute('data-render-model-type', 'real-source-monolithic');
  await expect(canvas).toHaveAttribute('data-render-error', '');
  await page.screenshot({ path: 'test-results/jack-in-the-box-v4-real-cc0-ready.png', fullPage: false });
  ...
  await page.screenshot({ path: 'test-results/jack-in-the-box-v4-real-cc0-released.png', fullPage: false });

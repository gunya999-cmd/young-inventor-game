...
test('outlet switch reliably powers on when the falling ball reaches the rocker', async ({ page }) => {
  ...
  await expect(canvas).toHaveAttribute('data-asset-version', 'outlet-switch-v4-reliable');
  await expect(canvas).toHaveAttribute('data-powered', 'false');
  await expect(canvas).toHaveAttribute('data-switch-latched', 'off');
  await expect(canvas).toHaveAttribute('data-physics', 'planck-falling-ball+reliable-impact-zone+motor-latched-switch-v4');
  ...
  await expect(canvas).toHaveAttribute('data-powered', 'true', { timeout: 10_000 });
